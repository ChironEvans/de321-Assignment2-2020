...
class MongoCursor:
    ...
    async def add_entry(self, name, payload):
        ...
        if self.connection():
            query = await self.db.jparser.analysed.update_one({'name': name}, {'$set': {'data': payload}}, upsert=True)
    ...
    async def fetch_entry(self, name):
        ...
        if self.connection():
            if not self.db.jparser.analysed.find_one({'name': name}):
                ...
                query = await self.db.jparser.analysed.find_one({'name': name})
                return query
        else:
            return False

    async def delete_entry(self, name):
        """Deleted an entry from the database, using the name given to the entry by the user"""
        if self.connection():
            if not self.db.jparser.analysed.find_one({'name': name}):
                return False
            else:
                query = await self.db.jparser.analysed.delete_one({'name': name})
                return query
        else:
            return False
